...
const CustomerNum: React.FC<CustomerProps> = () => {
    ...
    <div className="mt-[0.89rem] flex h-fit w-full flex-col justify-start rounded-md bg-white pb-[1.56rem] pl-[4.56rem] pr-[5.06rem] pt-[1.63rem] drop-shadow-[0px_4px_4px_rgba(0,0,0,0.25)]">
      <div className="flex flex-row items-center justify-start">
        <h1 className="select-none font-sans text-[1.5rem] font-bold">
          ...
          NGÀY GẦN NHẤT: 
          <span className="text-blue-900 font-extrabold text-1.5rem">
            {sum(filterData(data, selectedOption))}
          </span>
        </h1>
        <div className="relative">
          <select
            className="font-sans text-[1rem] font-bold text-[#005B6F] ml-[1rem] flex h-[3rem] w-[13.3125rem] cursor-pointer select-none flex-row items-center justify-between rounded-md border border-[#DFE4EA] px-[1.25rem] py-[0.75rem] hover:bg-gray-200"
      ...
      <div>
        {filterData(data, selectedOption) ? <LineChart chart_data={filterData(data, selectedOption)} /> : null}
      </div>
    </div>
  );
}

const LineChart: React.FC<LineChartProps> = ({ chart_data }) => {
  const options = {
    responsive: true,
    plugins: {
      legend: {
  ...
  return <Line className="mt-[3.37rem] h-[26rem] w-[62.0625rem] self-center"
    options={options}
    data={data} />;
}
